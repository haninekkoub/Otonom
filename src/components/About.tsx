...
export default function About() {
  ...
  const y1 = useSpring(useTransform(scrollYProgress, [0.3, 0.55], [0, 50]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001,
  });
  const y2 = useSpring(useTransform(scrollYProgress, [0.4, 0.65], [0, 50]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001,
  });
    ...
    <motion.section className="bg-darkbackground px-[12.5vw] py-16">
      <motion.div className="flex justify-center items-end gap-28 pb-16 relative ">
        <motion.div className="w-[30vw] h-[30vw] mt-16 relative overflow-hidden">
          <motion.div
            className="w-[35vw] h-[35vw]  bg-no-repeat bg-cover absolute bottom-0"
