...
export default function Banner(){
  ...
  const scrollLeft = useSpring(useTransform(scrollYProgress, [0, 0.2], [100, -200]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001
  });
  const scrollCenter = useSpring(useTransform(scrollYProgress, [0, 0.5], [-400, 200]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001
  });
  const scrollRight = useSpring(useTransform(scrollYProgress, [0, 0.2], [-50, -300]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001
  });
  return (
    <section className="w-full h-[300px] bg-[#F2F2F5] overflow-hidden relative">
      <motion.div className="flex gap-6 justify-center items-center rotate-12">
        <motion.div className="flex flex-col justify-center items-center gap-6 grow" style={{ y: scrollLeft }}>
          {BannerImages[1].map((BannerImage, i) => {
            return (
            <motion.div key={i} className="w-full h-[200px]"  
        ...
        <motion.div className="flex flex-col justify-center items-center gap-6 grow" style={{ y: scrollCenter}}>
          {BannerImages[1].map((BannerImage, i) => {
            return (
            <motion.div key={i} className="w-full h-[200px]"  
              style={{ backgroundImage: `url(${BannerImage.url})`, 
        ...
        <motion.div className="flex flex-col justify-center items-center gap-6 grow" style={{ y: scrollRight}}>
